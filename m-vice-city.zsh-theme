...
then
    PR_GIT_UPDATE=1
    FMT_BRANCH="${PM_RST} on ${purple}%b%u%c${hotpink} ●${PR_RST}"
  else
    FMT_BRANCH="${PM_RST} on ${purple}%b%u%c${PR_RST}"
  fi
  zstyle ':vcs_info:*:prompt:*' formats       "${FMT_BRANCH}"
...
setopt prompt_subst
PROMPT="${turquoise}%n%{$reset_color%} in 
${hotpink}%~%{$reset_color%}\$(ruby_prompt_info)\$vcs_info_msg_0_${purple} 
〽️%{$reset_color%} "
